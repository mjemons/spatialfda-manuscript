...
## code with the help of claude.ai ## 
seed = 51773
set.seed(seed)
...
Patients <- c(10, 20, 40, 80, 160, 320, 640)

out <- lapply(Patients, function(nPatients){
  resDf <- data.frame()
  set.seed(nPatients)
  print(paste0("signal iteration ",nIm))
  g1 <- rpois(nPatients/2, lambda)
...
resFinal <- list("nIm" = resultsNim, "nPatients" = resultsnPatients)

saveRDS(resFinal, snakemake@output[["rds"]])